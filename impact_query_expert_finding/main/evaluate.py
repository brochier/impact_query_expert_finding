...
def run(parameters):
    ...
    eval_batch = impact_query_expert_finding.evaluation.batch.EvalBatch(dataset, dump_dir = working_dir, max_queries = parameters["max_queries"])
    ...
    impact_query_expert_finding.evaluation.visual.plot_evaluation(gathered_evaluations[model_name]["all"], prefix=model_name,
                                                     path_visuals=working_dir)

    # Plot ROC curves for each topic
    impact_query_expert_finding.evaluation.visual.plot_ROC_topics(gathered_evaluations[model_name]["topics"], prefix=model_name,
                                                     path_visuals=working_dir)
